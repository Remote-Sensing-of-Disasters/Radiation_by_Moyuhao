#在一个文件夹中的所有的天内，比较这一系列时段的图的指定类像元的同一 CERES 大气参数 在不同时刻的均值方差
# 202200220 本代码为在newnewdata里头做的代码，覆盖加里曼丹大部分地方
# 我只需要把BAND数据改为CERES就行了
...
def output_joint_array(fns):
    ...
    obj_origin = tqdm(obj_origin)
    for obj in obj_origin: #时序索引从0000到0800
        obj_origin.set_description('Processing date')
        utc = obj[-8:-4]
        ...
        ary[-1,:,:] = gdal.Open(r'E:\SmokeDetection\source\semi-supervised learning\pixel classification results\Noon256batchsize3e4lrBN050Dropout\{}\{}.tif'.format(day1,utc)).ReadAsArray()
        ary[6,:,:] = gdal.Open(r'E:\Radiative Effect\ini_toa_sw_all_1h\{}\{}.tif'.format(day1,utc)).ReadAsArray()
        ary[7, :, :] = gdal.Open(r'E:\Radiative Effect\ini_sfc_sw_down_all_1h\{}\{}.tif'.format(day1,utc)).ReadAsArray()
        ary[8, :, :] =gdal.Open(r'E:\Radiative Effect\ini_toa_lw_all_1h\{}\{}.tif'.format(day1,utc)).ReadAsArray()
        ary[9, :, :] =gdal.Open(r'E:\Radiative Effect\adj_sfc_uva_all_1h\{}\{}.tif'.format(day1,utc)).ReadAsArray()
        for file in same_time_files[1:]:
            ...
            if os.path.exists(file):
                ...
                t[-2,:,:] = land_sea_label
                t[-1,:,:] = gdal.Open(r'E:\SmokeDetection\source\semi-supervised learning\pixel classification results\Noon256batchsize3e4lrBN050Dropout\{}\{}.tif'.format(day,utc)).ReadAsArray()
                t[6, :, :] = gdal.Open(
                    r'E:\Radiative Effect\ini_toa_sw_all_1h\{}\{}.tif'.format(day, utc)).ReadAsArray()
                t[7, :, :] = gdal.Open(
                    r'E:\Radiative Effect\ini_sfc_sw_down_all_1h\{}\{}.tif'.format(day, utc)).ReadAsArray()
                t[8, :, :] = gdal.Open(
                    r'E:\Radiative Effect\ini_sfc_lw_down_all_1h\{}\{}.tif'.format(day, utc)).ReadAsArray()
                t[9, :, :] = gdal.Open(
                    r'E:\Radiative Effect\adj_sfc_uva_all_1h\{}\{}.tif'.format(day, utc)).ReadAsArray()
                ary = np.c_[ary,t]
        imgs.append(ary)
    return imgs
...
CERES_names = [9,10] #78910波段被篡改，分别对应地表下行长波、地表下行短波、层顶长波、层顶短波
CERES_names = tqdm(CERES_names)
for name in CERES_names:
    CERES_names.set_description('Processing Radiation Parameter:')
    output_one_line(datasets,name,'smoke')
    output_one_line(datasets,name,'cloud')
    output_one_line(datasets,name,'sea')
    output_one_line(datasets,name,'land')
    plt.xlabel('UTC')
    if name>=7:
        plt.ylabel('W/m²')
    else:
    ...
    if name ==7:
        plt.title('TOA short wave')
    if name ==8:
        plt.title('Surface downward short wave')
    if name ==9:
        plt.title('Surface downward long wave')
    if name ==10:
        plt.title('UV adj_sfc_uva_all_1h')
    plt.legend()  # bbox_to_anchor=(0.1,0.5)
    plt.savefig(r'E:\Radiative Effect\Picture\radiative effect {}.jpg'.format(name))
    plt.close()
